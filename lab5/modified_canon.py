...
class ball():
    ...
    def hittest(self, obj):
        ...
        """
        distance_squared = (self.x - obj.x) ** 2 + (self.y - obj.y) ** 2
        if distance_squared < (self.r + obj.r) ** 2:
            return True
        else:
            return False
        ...
        for b in balls:
            ...
            for tar in targets:
                if b.hittest(tar) and tar.live:
                    ...
                    frags+=1
                    canv.itemconfig(screen1, text='Вы уничтожили цель ' + str(frags) + ' раз за ' + str(bullet) + ' выстрелов, ваша результативность равна ' +str(int(frags/bullet*100)) + '%')
                    t1=target()
                    t1.new_target()
                    targets.append(t1)
        canv.update()
